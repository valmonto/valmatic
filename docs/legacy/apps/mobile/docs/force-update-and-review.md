...
- Recommended for a *true* force-update: fetch the minimum from your API at runtime
  and pass it in — so you can gate an old client **without shipping a new build**:
  ```tsx
  <ForceUpdateGate minVersion={config?.minAppVersion} appStoreId="123456789">…</ForceUpdateGate>
  ```
...
- The OS **throttles** how often the prompt actually shows (a few times a year max), so
  calling `requestReview()` is a *request*, not a guarantee — don't gate anything on it.
