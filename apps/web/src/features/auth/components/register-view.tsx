...
export const RegisterView = () => {
  ...
  const { execute: register, isLoading: isApiLoading, error } = useRegister();
  ...
  return (
    <AuthShell title={t(k.auth.createAccount)} description={t(k.auth.getStartedWithWorkspace)} wide>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {capsLockOn && <p className="text-sm text-yellow-600">{t(k.auth.capsLockWarning)}</p>}
