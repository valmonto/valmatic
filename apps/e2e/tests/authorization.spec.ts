...
async function authGet(request: APIRequestContext, url: string, cookies: string) {
  return request.get(url, {
    headers: { Cookie: cookies },
  });
}

async function authPost(request: APIRequestContext, url: string, cookies: string, data?: unknown) {
...
async function authPatch(request: APIRequestContext, url: string, cookies: string, data?: unknown) {
...
async function authDelete(request: APIRequestContext, url: string, cookies: string) {
...
test.describe('Authorization - Protected Routes Without Token', () => {
  ...
  test('GET /api/notifications without token returns 401', async ({ request }) => {
    const response = await request.get(`${getApiUrl()}/api/notifications`);
    expect(response.status()).toBe(401);
  });

  test('POST /api/auth/logout without token returns 401', async ({ request }) => {
  ...
  test('Request with malformed Authorization header returns 401', async ({ request }) => {
    const response = await request.get(`${getApiUrl()}/api/auth/me`, {
      headers: {
...
test.describe('Authorization - Role-Based Access Control', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    // Login as owner
    ownerCookies = await login(request, TEST_USERS.owner.email, TEST_USERS.owner.password);
    ...
    // Login as admin
    adminCookies = await login(request, TEST_USERS.admin.email, TEST_USERS.admin.password);
    ...
    // Login as member
    memberCookies = await login(request, TEST_USERS.member.email, TEST_USERS.member.password);
  });

  test('OWNER can list users', async ({ request }) => {
    const response = await authGet(request, `${getApiUrl()}/api/users`, ownerCookies);
    expect(response.status()).toBe(200);
  });

  test('ADMIN can list users', async ({ request }) => {
    const response = await authGet(request, `${getApiUrl()}/api/users`, adminCookies);
    expect(response.status()).toBe(200);
  });

  test('MEMBER cannot list users (403)', async ({ request }) => {
    const response = await authGet(request, `${getApiUrl()}/api/users`, memberCookies);
    expect(response.status()).toBe(403);
  });

  test('MEMBER cannot create users (403)', async ({ request }) => {
    const response = await authPost(request, `${getApiUrl()}/api/users`, memberCookies, {
  ...
  test('ADMIN cannot create OWNER users (403)', async ({ request }) => {
    const response = await authPost(request, `${getApiUrl()}/api/users`, adminCookies, {
  ...
  test('OWNER can create OWNER users', async ({ request }) => {
    const response = await authPost(request, `${getApiUrl()}/api/users`, ownerCookies, {
  ...
  test('All roles can access /api/auth/me', async ({ request }) => {
    const ownerMe = await authGet(request, `${getApiUrl()}/api/auth/me`, ownerCookies);
    const adminMe = await authGet(request, `${getApiUrl()}/api/auth/me`, adminCookies);
    const memberMe = await authGet(request, `${getApiUrl()}/api/auth/me`, memberCookies);
  ...
  test('All roles can access notifications', async ({ request }) => {
    const ownerNotif = await authGet(request, `${getApiUrl()}/api/notifications`, ownerCookies);
    const adminNotif = await authGet(request, `${getApiUrl()}/api/notifications`, adminCookies);
    const memberNotif = await authGet(request, `${getApiUrl()}/api/notifications`, memberCookies);
...
test.describe('Authorization - Tenant Isolation', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    org1OwnerCookies = await login(request, 'org1-owner@test.local', 'TestPassword123!');
    ...
    if (createUserResponse.ok()) {
      ...
      const usersResponse = await authGet(request, `${getApiUrl()}/api/users`, org1OwnerCookies);
      const users = await usersResponse.json();
      const user = users.data?.find((u: { email: string }) => u.email === 'org1-user@test.local');
      org1UserId = user?.id;
    }
    ...
    org2OwnerCookies = await login(request, 'org2-owner@test.local', 'TestPassword123!');
  });

  test('Org2 owner cannot list Org1 users', async ({ request }) => {
    // Org2 owner should only see their own org's users
    const response = await authGet(request, `${getApiUrl()}/api/users`, org2OwnerCookies);
  ...
  test('Each org only sees their own data', async ({ request }) => {
    const org1Users = await authGet(request, `${getApiUrl()}/api/users`, org1OwnerCookies);
    const org2Users = await authGet(request, `${getApiUrl()}/api/users`, org2OwnerCookies);

    const org1Data = await org1Users.json();
    const org2Data = await org2Users.json();

    const org1Emails = org1Data.data?.map((u: { email: string }) => u.email) || [];
    const org2Emails = org2Data.data?.map((u: { email: string }) => u.email) || [];
...
test.describe('Authorization - Session Management', () => {
  test('Logout invalidates session', async ({ request }) => {
    ...
    const meBeforeLogout = await authGet(request, `${getApiUrl()}/api/auth/me`, cookies);
    ...
    const meAfterLogout = await authGet(request, `${getApiUrl()}/api/auth/me`, cookies);
