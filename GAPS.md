...
| E2E never ran anywhere                                    | Runs as the production deploy gate (`deploy.yml`: verify → e2e → deploy) and on demand via workflow_dispatch. Deliberately NOT per-PR: the ~8 min of Docker builds would triple PR feedback time, and the failure it catches matters exactly at deploy |
